			...
			for(intc=0; c<board[0].length; c++){
				for(int r=0; r<board.length; r++){
					if(board[r][c].equalsIgnoreCase("A")){
						...
						for(int i=0; i+r<board.length; i++){
							if(board[r+i][c].equalsIgnoreCase("B")){
								counter++;
							}
							else{
								counter=0;
	...
	public static int checkHorizontal(String[][] board){
		...
		for(int r=0; r<board.length; r++){
			for(int c=0; c<board[0].length; c++){
				if(board[r][c].equalsIgnoreCase("A")){
					for(int i=0; i+r<board.length; i++){
						if(board[r][c+1].equalsIgnoreCase("A")){
							counter++
						}
						else{
							counter=0;
					...
					for(int i=0; i+r<board.length; i++){
						if(board[r][c+1].equalsIgnorease("B")){
							counter++
						}
						else{
							counter=0;
	...
	public static int checkDiagonal(String[][] board){
		...
		for(int r=0; r<board.length; r++){
			...
			for(int c=0; c<board[0].length; c++){
				x=c;
				if(board[r][c].equalsIgnoreCase("A")){
					while(x+1<board[0].length && y-1>=0){
						if(board[y-1][x+1].equalsIgnoreCase("A")){
							counter++;
						}
						else{
							counter=0;
			...
			if(board[r][c].equalsIgnoreCase("B")){
				while(x+1<board[0].length && y-1>=0){
					if(board[y-1][x+1].equalsIgnoreCase("B")){
						counter++;
					}
					else{
						counter =0;
